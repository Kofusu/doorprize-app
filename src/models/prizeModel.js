...
module.exports.getAllPrizeBySession = async () => {
  const query = "SELECT prize.id_prize, prize.nama_prize, prize.nama_gambar, sessions.id_session, sessions.nama_session FROM sessions inner join prize on sessions.id_session=prize.id_session"
...
module.exports.addPrize = async (caption, fName, sessionId, unit) => {
  const query = "INSERT INTO prize(nama_prize, nama_gambar, id_session, max_winner) value(?, ?, ?, ?)"
  return await sequelize.query(query, {
